...
public class MBEzRelease {
	...
	{
		try{artistName=r.artistCredit.nameCredit.artist.name;}catch(Exception ex){};
		try{artistSortName=r.artistCredit.nameCredit.artist.sortName;}catch(Exception ex){};
		try{artistId=r.artistCredit.nameCredit.artist.id;}catch(Exception ex){};
		try{artistDisamb=r.artistCredit.nameCredit.artist.disambiguation;}catch(Exception ex){};
		country=r.country;
		date=r.date;
		id=r.id;
		try{label=r.labelInfoList.elementAt(0).label.name;}catch(Exception ex){};
		title=r.title;
		
		try{format=r.mediumList.elementAt(0).format;}catch(Exception ex){};
